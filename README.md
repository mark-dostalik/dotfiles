...
2. Copy the snapshot from your temporary directory to the correct locations on your new machine.
   ...
   ```
   This command copies every file in `dotfiles-tmp` and its subdirectories into the corresponding locations in your home directory (so `dotfiles-tmp/.zshenv` will be copied to `~/.zshenv`).
...
[^1]: [Credit.](https://stegosaurusdormant.com/bare-git-repo/#copying-your-dotfiles-to-another-machine)
